...
serve(async (req) => {
  ...
  try {
    ...
    if (!uid) return Response.json({ error: "Unauthorized" }, { status: 401, headers: corsHeaders });

    const { data: access } = await sbAdmin.rpc("has_pro_access", { uid });
    if (!access) return Response.json({ error: "Upgrade required", upgrade: true }, { status: 402, headers: corsHeaders });
    ...
    if (case_id) {
      ...
      const { data: one } = await sbAdmin.from("export_cases").select("id").eq("org_id", org_id).order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (one?.id) await sbAdmin.from("case_events").insert({ case_id: one.id, event_type: "analysis", payload: result });
    }
