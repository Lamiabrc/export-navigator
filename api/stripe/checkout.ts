...
async function safeReadJson<T>(req: VercelRequest): Promise<T> {
  // ✅ si body vide / pas JSON => on retourne un objet vide
  try {
...
export default allowCors(async (req: VercelRequest, res: VercelResponse) => {
  try {
    ...
    if (missing.length) {
      json(res, 500, { ok: false, error: "Missing env vars", missing });
      return;
    }
    ...
    if (userError || !userData?.user) {
      json(res, 401, { ok: false, error: "Invalid auth token", detail: userError?.message || null });
      return;
    }

    const user = userData.user;

    // ✅ Tolérant si body vide
    ...
    if (!stripePriceId.startsWith("price_")) {
      json(res, 400, { ok: false, error: "Invalid price id", detail: "Expected a Stripe price_... id" });
      return;
    }
    ...
    if (customerError) {
      json(res, 500, {
        ok: false,
        error: "Failed to read billing customer",
    ...
    if (!stripeCustomerId) {
      ...
      if (upsertError) {
        json(res, 500, {
          ok: false,
          error: "Failed to store billing customer",
    ...
    // ✅ Catch-all: renvoie enfin l’erreur réelle au lieu d’un 500 “vide”
